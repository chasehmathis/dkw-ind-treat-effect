...
get_bounds <- function(m = 200, alpha = 0.1, nsim = 1e3) {
  ...
  generate_true_sample <- function(m) {
    ...
    design <-
      declare_model(
        N = m, 
        potential_outcomes(Y ~ rlnorm(N, mean = 2*Z, sd = 0.4))
...
get_marakov_bounds_all_t <- function() {
  ...
  plot.ecdf(data$tau, main = "true ECDF of invidual tau")
